...
$action="<a href=\"$link\" >".Yii::t("default","Details")."</a>";
	...
	{		
        ...
        if($this->data['merchant_type']==2 && Yii::app()->functions->getOptionAdmin('admin_enabled_iyzipay')=="yes")
        {
          //ADD AS SUB MERCHANT
  		    $sub_msg = FunctionsV3::addSubmerchant($mtid,$this->data);
  		    if(strstr($sub_msg, 'ERROR'))
  		    {
            $this->msg=$sub_msg;
  	    		return ;
          }
          ...
          if($this->data['merchant_type']==2 && Yii::app()->functions->getOptionAdmin('admin_enabled_iyzipay')=="yes"){          
  			    $params_sub=array(
        		   'merchant_id'=>$mtid,
        		   'submerchant_type'=>$this->data['submerchant_type'],
        		   'seller_name'=>$this->data['restaurant_name'],
        		   'gsm_number'=>$this->data['contact_phone'],
        		   'iban'=>$this->data['iban'],
        		   'sub_contact_name'=>isset($this->data['contact_name'])?$this->data['contact_name']:'',
        		   'sub_contact_surname'=>isset($this->data['sub_contact_surname'])?$this->data['sub_contact_surname']:'',
        		   'identity_number'=>isset($this->data['identity_number'])?$this->data['identity_number']:'',
        		   'tax_office'=>isset($this->data['tax_office'])?$this->data['tax_office']:'',
        		   'tax_number'=>isset($this->data['tax_number'])?$this->data['tax_number']:'',
        		   'legal_company_title'=>isset($this->data['legal_company_title'])?$this->data['legal_company_title']:'',
        		   'submerchant_key'=>$sub_msg
        		);
        		$this->insertData("{{merchant_sub}}",$params_sub);
      		}
			    // end
