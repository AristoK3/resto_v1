...
/*	public static function getItemFirstPrice($prices='',$discount='')
    ...
    public static function getAdminPaymentList()
    {
    	...
    	if (getOptionA('admin_enabled_iyzipay')=="yes"){
    		$payment_available[]='iyzipay';
    	}
	...
	public static function getMerchantPaymentListNew($merchant_id='')
	{
		...
		switch ($merchant_type) {			
			...
			default:
		    	...
		    	if ( Yii::app()->functions->getOptionAdmin('admin_enabled_iyzipay')=="yes"){
					$payment_available[]='iyzipay';
			    }
   ...
   public static function validateVoucherByAddress($voucher_code='', $street='', $city='', $state='')
   {
	...
	public static function SubmerchantType()
  {
  	return array(
  	  'PERSONAL'=>t("Personal"),
  	  'PRIVATE_COMPANY'=>t("Private"),
  	  'LIMITED_OR_JOINT_STOCK_COMPANY'=>t("Limited Company")
  	);
  }  
  
  public static function addSubmerchant($mtid='', $data='')
   {
        ...
        $request->setSubMerchantExternalId(uniqid());
        $request->setLocale(\Iyzipay\Model\Locale::TR);
        $request->setConversationId("1111111");          
        $request->setAddress($data['street'].' '.$data['city'].' '.$data['post_code'].' '.$data['state']);
        if($data['submerchant_type'] == "PERSONAL")
        {
          $request->setSubMerchantType(\Iyzipay\Model\SubMerchantType::PERSONAL);
          $request->setContactName(str_replace(array(",","'"),array('',''),$data['contact_name']));
          $request->setContactSurname(str_replace(array(",","'"),array('',''),$data['sub_contact_surname']));
          $request->setIdentityNumber($data['identity_number']);
        }
        else if($data['submerchant_type'] == "PRIVATE_COMPANY")
        {
          $request->setSubMerchantType(\Iyzipay\Model\SubMerchantType::PRIVATE_COMPANY);
          $request->setTaxOffice(str_replace(array(",","'"),array('',''),$data['tax_office']));
          $request->setLegalCompanyTitle(str_replace(array(",","'"),array('',''),$data['legal_company_title']));
          $request->setIdentityNumber($data['identity_number']);
        }
        else if($data['submerchant_type'] == "LIMITED_OR_JOINT_STOCK_COMPANY")
        {
        ...
        $request->setEmail($data['contact_email']);
        $request->setGsmNumber($data['contact_phone']);
        $request->setName(str_replace(array(",","'"),array('',''),$data['restaurant_name']));
        ...
        {
          $error = '<strong>ERROR</strong>: Please solve this iyzico error to become sub-merchant: '.$subMerchant->getErrorMessage();
